...
test -f /var/www/.toolkit/sql.php || wget -O /var/www/.toolkit/sql.php -c $(curl -kLv https://github.com/vrana/adminer/releases/latest|grep php|grep releases/download|grep [0-9]/adminer.*[0-9].php|cut -d\" -f2|sed 's/^/https:\/\/github.com/g' 2>/dev/shm/startlogs/toolkit.sql.version.log ) &>/dev/shm/startlogs/toolkit.sql.log && echo put sql mgmt adminer 
