...
_supervisor_generate_artisanqueue() { ###supervisor queue:work
                    ...
                    for artisanfile in $(find /var/www -maxdepth 2 -name artisan 2>/dev/null|grep -v  -e "\.bak/artisan" -e "OLD/artisan" -e  "old/artisan"  |head -n1 ) ;do
                        ...
                        #
                        test -e /dev/shm/.notified.queuedriver || grep -e QUEUE_CONNECTION=sync -e QUEUE_DRIVER=sync  $(dirname $artisanfile)/.env -q && { sleep 20; echo "  sys.hint | NOT ENABLING SUPERVISOR ARTISAN QUEUE BECAUSE QUEUE=sync in .env" ; touch /dev/shm/.notified.queuedriver ; } &
