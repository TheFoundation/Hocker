...
trap 'echo Sorry .. hanging up;kill -s SIGTERM $(supervisorctl pid) $(pidof supervisord) & sleep 15  ; exit;' SIGINT
trap 'echo Sorry .. hanging up;kill -s SIGTERM $(supervisorctl pid) $(pidof supervisord) & sleep 15  ; exit;' SIGTERM
...
_supervisor_generate_artisanqueue() { ###supervisor queue:work
                    ...
                    for artisanfile in $(find /var/www -maxdepth 2 -name artisan 2>/dev/null|grep -v  -e "\.bak/artisan" -e "OLD/artisan" -e  "old/artisan"  |head -n1 ) ;do
                        ...
                        #
                        test -e /dev/shm/.notified.queuedriver || grep -e QUEUE_CONNECTION=sync -e QUEUE_DRIVER=sync  $(dirname $artisanfile)/.env -q && { sleep 20; echo "  sys.hint | NOT ENABLING SUPERVISOR ARTISAN QUEUE BECAUSE QUEUE=sync in .env" |lightblue; touch /dev/shm/.notified.queuedriver ; } &

                        grep -q -e QUEUE_CONNECTION=sync -e QUEUE_DRIVER=sync  $(dirname $artisanfile)/.env  && test -e /etc/supervisor/conf.d/queue_${artisanfile//\//_}.conf || php ${artisanfile} 2>&1 |grep -q queue:work  && test -e $(dirname $artisanfile)/.env &&  grep -e QUEUE_CONNECTION=sync -e QUEUE_DRIVER=sync  $(dirname $artisanfile)/.env ||  (
...
exec $(which supervisord || echo /usr/bin/supervisord) -c /etc/supervisor/supervisord.conf 2>&1 | awk '!NF || !seen[$0]++' | sed 's/^[[:digit:]]\{4\}-[[:digit:]]\{2\}-[[:digit:]]\{2\} [[:digit:]]\{2\}:[[:digit:]]\{2\}:[[:digit:]]\{2\},[[:digit:]]\{3\} [[:upper:]]/ sys.init  | \0/g'  > /dev/stdout
