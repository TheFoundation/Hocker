...
if [ "$(ls -1 /usr/sbin/php-fpm* 2>/dev/null|wc -l)" -eq 0 ];then
    ...
    grep ^'php_admin_value\[disable_functions\] = '  /etc/php/$(php --version|head -n1|cut -d" " -f2|cut -d\. -f 1,2)/fpm/pool.d/www.conf  ||  {
      ...
      FORBIDDEN_FUNCTIONS_DEFAULTS="system,passthru,shell_exec,chroot,system,proc_open,popen,parse_ini_file,show_source,chroot,shell_exec,proc_open,proc_get_status,ini_restore,ftp_connect,ftp_exec,ftp_get,ftp_login,ftp_nb_fput,ftp_put,ftp_raw"
