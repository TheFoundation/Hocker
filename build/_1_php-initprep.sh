...
if [ "$(ls -1 /usr/sbin/php-fpm* 2>/dev/null|wc -l)" -eq 0 ];then
    ...
    grep ^'php_admin_value\[disable_functions\]'  /etc/php/$(php --version|head -n1|cut -d" " -f2|cut -d\. -f 1,2)/fpm/pool.d/www.conf  ||  {
      ...
      FORBIDDEN_FUNCTIONS_HARDENED="system,exec,passthru,shell_exec,system,proc_open,popen,parse_ini_file,show_source,chroot,escapeshellcmd,escapeshellarg,shell_exec,proc_open,proc_get_status,ini_restore,ftp_connect,ftp_exec,ftp_get,ftp_login,ftp_nb_fput,ftp_put,ftp_raw"
      FORBIDDEN_FUNCTIONS_DEFAULTS="system,passthru,shell_exec,chroot,system,popen,parse_ini_file,show_source,chroot,shell_exec,ini_restore,apache_child_terminate,apache_setenv,ftp_connect,ftp_exec,ftp_get,ftp_login,ftp_nb_fput,ftp_put,ftp_raw"
    ...
    ##write
      (echo;echo "php_admin_value[disable_functions] = "${FORBIDDEN_FUNCTIONS_DEFAULTS}) >> /etc/php/$(php --version|head -n1|cut -d" " -f2|cut -d\. -f 1,2)/fpm/pool.d/www.conf
         echo -n ; } ;
...
sed 's/.\+/access.log/AccessLog | /bin/bash /_3_logfilter_apache.sh >> \/dev\/stdout/g'  -i /etc/apache2/sites-enabled/*.conf  ;
sed 's/.\+\/access.log/CustomLog | /bin/bash /_3_logfilter_apache.sh >> \/dev\/stdout/g' -i /etc/apache2/sites-enabled/*.conf  ;
sed 's/.\+\/error.log/ErrorLog | /bin/bash /_3_logfilter_apache.sh >> \/dev\/stderr/g'   -i /etc/apache2/sites-enabled/*.conf  ;
