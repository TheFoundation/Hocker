...
if [ ! -f ${SSH_KEY_ED25519} ]; then
    dropbearkey  -t ed25519 -f ${SSH_KEY_ED25519}    ;    chown root:root          ${SSH_KEY_ED25519};    chmod 600                ${SSH_KEY_ED25519}
fi &

if [ ! -f ${SSH_KEY_ECDSA} ]; then
    dropbearkey  -t ecdsa -f ${SSH_KEY_ECDSA} -s 521 ;    chown root:root          ${SSH_KEY_ECDSA}  ;    chmod 600                ${SSH_KEY_ECDSA}
fi &

if [ ! -f ${SSH_KEY_RSA} ]; then
    dropbearkey  -t rsa -f ${SSH_KEY_RSA} -s 4096    ;    chown root:root          ${SSH_KEY_RSA}    ;    chmod 600                ${SSH_KEY_RSA}
fi &
