...
###### PHP IMAGICK
    ...
    if [ "${build_php_imagick}" = "true" ] ;then 
        ...
        apt-get -y  install build-essential   php${PHPVersion}-dev pkg-config
        #echo |pecl install imagick
        _build_pecl imagick && echo extension=imagick.so > /etc/php/${PHPVersion}/mods-available/redis.ini && phpenmod imagick
