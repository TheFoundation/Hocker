...
env|grep APT|sed 's/[0-9]\+\./x./g;s/[0-9]\+\:/x:/g'
...
if [ "x$APT_HTTP_PROXY_URL" = "x" ] ; then
    ...
    ( echo 'Acquire::http::Proxy "'${APT_HTTP_PROXY_URL}'/";' ;echo 'PassThroughPattern: ^(.*):443$;';)| tee /etc/apt/apt.conf.d/10-proxy |sed 's/[0-9]\+\./x./g;s/[0-9]\+\:/x:/g'   ;
fi
