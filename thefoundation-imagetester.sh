...
cat ${apaconfig} | grep  "AccessLog"|wc -l | grep -q ^0 || grep "AccessLog" ${apaconfig} |  grep -q "stdout" || {  echo -n " apache_log_not_stdout_"$(echo ${apaconfig//.conf}|sed 's/.\+\///g' )  >> /dev/shm/apache_fails ;
                                                  echo;echo "FAIL( missing  AccessLog STDOUT redirect in $apaconfig " |red ;echo "###";grep AccessLog ${apaconfig}; echo "###" ; }  && { echo -n "OK:ACSLOG:"|green ;echo -n $(basename $apaconfig ) ; } ;
cat ${apaconfig} | grep  "CustomLog"|wc -l | grep -q ^0 || grep "CustomLog" ${apaconfig} |  grep -q "stdout" || {  echo -n " apache_log_not_stdout_"$(echo ${apaconfig//.conf}|sed 's/.\+\///g' )  >> /dev/shm/apache_fails ;
                                                  echo;echo "FAIL( missing  CustomLog STDOUT redirect in $apaconfig " |red ;echo "###";grep CustomLog ${apaconfig}; echo "###" ; }  && { echo -n "OK:CSTLOG:"|green ;echo -n $(basename $apaconfig ) ; } ;
cat ${apaconfig} | grep  "ErrorLog" |wc -l | grep -q ^0 || grep "ErrorLog"  ${apaconfig} |  grep -q "stderr" || {  echo -n " apache_errlog_not_stderr_"$(echo ${apaconfig//.conf}|sed 's/.\+\///g' )  >> /dev/shm/apache_fails ;
                                                  echo;echo "FAIL( missing   ErrorLog STDERR redirect in $apaconfig " |red ;echo "###";grep ErrorLog ${apaconfig} ; echo "###" ; }  && { echo -n "OK:ERRLOG:"|green ;echo -n $(basename $apaconfig ) ; } ;
done
...
echo "################"
echo  -n "MAILS:";echo  $(echo " | sendmail: " $(which sendmail && file $(which sendmail|cut -d, -f1) ))
